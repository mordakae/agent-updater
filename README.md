...
Each level has:
- A **manifest** (`manifest.yml`) listing packages: their git `url`, `branch` to track, the `sha` the user was last notified about, the `applied_sha` actually applied locally, whether the two are `in_sync`, a `selection` (which of the package's Rules/Skills/Agents/Other pieces to install — see below), `installed_files` (a `{source, target, category, item}` map used to apply and later reverse a package's changes cleanly), and — only for a package that upgrades itself — an `upgrade` override (see below).
...
### Packages that upgrade themselves

Not every package is a set of config files to copy in. Some — Spec Kit is the obvious example — ship their own scaffolding CLI or update script that generates the files itself. The tempting behaviour is to hand you back to it ("run `specify upgrade` when you want to update"), but that quietly gives up the whole point: the upgrade now only happens when *you* remember to trigger it, which is the chore this tool exists to remove.

So those packages get an **upgrade override** instead. At install the agent spots that the package upgrades itself, shows you its install and upgrade commands, and asks once for approval to run them on your behalf. The upgrade command is recorded on the manifest entry (`upgrade: {command, requires}`), and from then on the due-check runs it for you on your normal cadence — the tracked repo still answers *whether* an upgrade is due, the package's own command does the applying. `selection`/`installed_files` don't apply, since those files belong to the package's tool.

The guardrails: approval is re-asked if the package changes its command upstream, so a package can't swap in a new one and have it run unattended. If the command needs external tooling (`uv`, `npm`, `brew`) that isn't installed, the agent never installs it silently — it tells you what's missing and helps you resolve it, leaving the package pending until it's sorted. And since a repo-level manifest is often committed, the command recorded in it says only *what* it is, never that it was approved: approval is per-machine, in a gitignored `approvals.local.yml`, so a teammate's approval never runs a command unattended in your session. (The user level needs none of that — its manifest is already yours alone.)
