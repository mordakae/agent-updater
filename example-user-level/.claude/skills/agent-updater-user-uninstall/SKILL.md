...
If the entry has an `upgrade` block, its files were generated by the package's own tool and aren't in `installed_files`, so this skill can't reverse them. Check the package's docs for an uninstall/clean command; if there is one, show it to the user and run it only with their explicit approval. If there isn't, say which files are being left behind. Either way, still delete the checkout and the manifest entry.
