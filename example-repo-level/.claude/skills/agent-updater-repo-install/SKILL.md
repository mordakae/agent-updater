...
- If approved:
  - Record the command on the package's manifest entry:
    ...
    ```
  - Record the approval in `agent-updater/approvals.local.yml` as `<package-name>: <the exact approved command string>`, and confirm that file is excluded from version control as `agent-updater/packages/` is. See the `agent-updater-repo` skill's *Approval is per-machine* for why it lives outside the manifest.
