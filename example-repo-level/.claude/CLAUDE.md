...
- Check the update history.
  - Read `agent-updater/update_history.yml` (repo root). It has two fields:
  ...
  - If the number of days between `last_update` and the current date is greater than or equal to the mapped `update_freq` threshold (or `last_update` is blank), invoke the `agent-updater-repo` skill to check for and apply package updates.
